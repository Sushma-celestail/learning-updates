...
# Every step from receiving the user question to returning the final answer
# is logged so the full pipeline trace is visible in rag_pipeline.log.

import time    # Used to measure latency at each pipeline step
...
def format_docs(docs):
    """
    Convert a list of retrieved LangChain Document objects into a single
    context string that includes numbered source citations.

    This function is called inside the LCEL pipeline after the retriever
    returns the top-k chunks.  The formatted string is injected into the
    prompt as {context}.

    Parameters
    ----------
    docs : list[Document]
        Documents returned by the ChromaDB retriever.

    Returns
    -------
    str
        Multi-block string where each block starts with a [Source N: URL]
        header followed by the chunk text.
    """
    ...
    for index, doc in enumerate(docs, start=1):
        source = doc.metadata.get("source", "Unknown source")  # Page URL stored at ingest time
        chunk_preview = doc.page_content[:80].replace("\n", " ")  # First 80 chars for the log

        log.debug(
            "  Chunk %d | source: %s | preview: %s...",
            index, source, chunk_preview
        )

        # Build the formatted block that the LLM will read as context
        formatted_chunks.append(f"[Source {index}: {source}]\n{doc.page_content}")

    context = "\n\n".join(formatted_chunks)   # Separate chunks with blank lines
