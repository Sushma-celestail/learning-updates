      ...
      <div className="glass-card flex flex-col gap-6 p-8">
        <div className="flex gap-4">
          <div className="flex-1 space-y-2">
            <label className="text-sm font-medium text-muted">Celsius (°C)</label>
            <input type="number" value={celsius} onChange={e => handleCChange(e.target.value)} placeholder="0" className="text-2xl text-center py-4" />
          </div>
          <div className="flex-1 space-y-2">
            <label className="text-sm font-medium text-muted">Fahrenheit (°F)</label>
            <input type="number" value={fahrenheit} onChange={e => handleFChange(e.target.value)} placeholder="32" className="text-2xl text-center py-4" />
          </div>
        </div>
        <div className="text-center p-6 bg-white/5 rounded-xl border border-white/10">
          <p className="text-muted text-sm uppercase tracking-widest mb-2 font-semibold">Water State</p>
          <p className={`text-4xl font-bold ${celsius >= 100 ? 'text-secondary' : celsius <= 0 ? 'text-accent' : 'text-success'}`}>
  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <h3 className="text-xl font-semibold text-muted">JSON Result</h3>
        <div className="glass-card bg-slate-900 font-mono text-sm overflow-hidden p-6">
          <pre className="text-accent underline decoration-primary/30">
            {JSON.stringify(submittedData || responses, null, 2)}
          </pre>
        </div>
        {!submittedData && <p className="text-xs text-muted italic">* Showing live state before submit</p>}
      </div>
    </div>
  );
};
